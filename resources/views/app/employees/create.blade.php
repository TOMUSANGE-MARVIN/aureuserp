...
@section('breadcrumb', 'New Employee')

@section('content')
    <div class="max-w-2xl">
        <div class="flex items-center justify-between mb-5">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">New Employee</h1>
            <a href="{{ route('employees.index') }}" class="px-4 py-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-colors">Cancel</a>
        </div>
        ...
        <form method="POST" action="{{ route('employees.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="employee_type"
                        ...
                        @foreach(['employee'=>'Employee','student'=>'Student','freelance'=>'Freelance','external'=>'External'] as $val => $label)
                            <option value="{{ $val }}" {{ old('employee_type') === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div>
                        ...
                        <select name="marital"
                            ...
                            @foreach(['single'=>'Single','married'=>'Married','cohabitant'=>'Cohabitant','widower'=>'Widower','divorced'=>'Divorced'] as $val => $label)
                                <option value="{{ $val }}" {{ old('marital') === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-sm font-medium transition-colors">Create Employee</button>
                <a href="{{ route('employees.index') }}" class="px-4 py-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-colors">Cancel</a>
            </div>
        </form>
    </div>
