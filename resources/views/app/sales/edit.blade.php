...
<nav class="text-sm text-gray-500 dark:text-gray-400">
    ...
    <span class="mx-2">/</span>
    <a href="{{ route('sales.show', $order->id) }}" class="hover:text-brand-600">{{ $order->name }}</a>
    <span class="mx-2">/</span>
...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update sales order details</p>
    </div>
</div>
...
<div class="max-w-2xl">
    <form method="POST" action="{{ route('sales.update', $order->id) }}">
        ...
        <div class="bg-white dark:bg-[#111118] rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 p-6 space-y-5">
            ...
            <div>
                ...
                <select name="state" class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1a1a27] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500">
                    ...
                    <option value="sale"  {{ old('state', $order->state) === 'sale'  ? 'selected' : '' }}>Confirmed</option>
            ...
            <div>
                ...
                <input type="text" name="client_order_ref" value="{{ old('client_order_ref', $order->client_order_ref) }}"
            ...
            <div>
                ...
                <input type="date" name="validity_date" value="{{ old('validity_date', $order->validity_date ? \Carbon\Carbon::parse($order->validity_date)->format('Y-m-d') : '') }}"
                       class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1a1a27] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500">
            ...
            <div>
                ...
                <textarea name="note" rows="4"
                          class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1a1a27] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500">{{ old('note', $order->note) }}</textarea>
        ...
        <div class="flex items-center gap-3 mt-5">
            ...
            <a href="{{ route('sales.show', $order->id) }}" class="px-4 py-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-colors">
                Cancel
            </a>
        </div>
    </form>
